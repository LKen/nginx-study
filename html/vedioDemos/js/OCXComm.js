...
define(function () {
   ...
   * @type {function}
   */
  OCXUtils.inheritance = function (child, parent) {
  ...
  OCXComm.prototype.getVersion = function () {
    ...
    // ret = this.sysFunc(paramJson);
    console.log('this.ocxObj', this.ocxObj)
    ret = this.ocxObj.GetVersion();
    return ret;
  };
  /**
   ...
   * @returns {Object}
   */
  OCXComm.prototype.deleteView = function () {
    var paramJson, ret;
    if (!this.ocxObj) {
      throw new Error("OCX对象尚未初始化");
    }
    paramJson = this.generateOcxParamJson("DeleteView", null);
    ret = this.sysFuncVideo(paramJson);
    return ret;
  };
  /**
   ...
   * @constructor
   * @extends OCXComm @see {@link OCXComm}
   */
   ...
   */
  OCXUtils.inheritance(SOCXComm, OCXComm);
  /**
   * @description 注册控件消息回调函数，用于控件消息回调给web
   * @override 复写父类的系统回调函数  @see {@link OCXComm#registerCallback}
   * @param{function} callbackFunc    回调函数
   */
   ...
   * @override 复写父类的系统函数  @see {@link OCXComm#sysFunc}
   * @param {object} paramJson    generateOcxParamJson生成的参数
   */
   ...
   * @param {String} strParam 复制的内容
   * @returns {Object}
   *  data.code = {number} 0,成功;其他，错误;
   *  data.action="UnSubCarInfo"
   */
  SOCXComm.prototype.copy2Clipboard = function (strParam) {
    var params = {}, paramJson, ret;
    if (!this.ocxObj) {
      throw new Error("OCX对象尚未初始化");
    }
    params.strParam = strParam;
    paramJson = this.generateOcxParamJson("Clipboard", params);
    ret = this.sysFunc(paramJson);
    return ret;
  };
  /**
  ...
  /**
   * @description 实时图片显示相关处理接口，包括实时图片窗口处理、打开和关闭实时图片浏览等
   * @param {String} paramJson    @see{@link OCXComm#generateOcxParamJson}生成的json数据
   */
   ...
   *  	nDevID = 70,
   *      nChlID = 4;
   *      openLivePic(nWndNo,nChlID,nDevID);
   * @param {Integer} nWndNo    图片显示窗口号
   * @param {Integer} nChlID   显示的通道ID
   * @param {Integer} nDevID   设备ID
   * @returns {Object}
   *  data.code = {number} 0,成功;其他，错误;
   *  data.action="OpenLivePic"
   */
  SOCXComm.prototype.openLivePic = function (nWndNo, nChlID, nDevID) {
    var params = {}, paramJson, ret;
    if (!this.ocxObj) {
      throw new Error("OCX对象尚未初始化");
    }
  ...
  /**
   * @description 订阅实时图片信息
   * @example <caption>Example usage of <b>subscribeLivePic</b>.</caption>
   *  var nWndNo = 1,
   *  	nDevID = 70,
   *  	nChlID = 4;
   *      subscribeLivePic(nWndNo);
   * @param {Integer} nWndNo   图片显示窗口号(0~100)
   * @param {String} nChlID	 通道ID
   * @param {String} nDevID	 设备ID
   * @returns {Object}
   *  data.code = {number} 0,成功;其他，错误;
   *  data.action="SubCarInfo"
   */
  SOCXComm.prototype.subscribeLivePic = function (nWndNo, nChlID, nDevID) {
    var params = {}, paramJson, ret;
    if (!this.ocxObj) {
      throw new Error("OCX对象尚未初始化");
    }
    ...
    params.nChlID = parseInt(nChlID);
    params.nDevID = parseInt(nDevID);
    paramJson = this.generateOcxParamJson("SubCarInfo", params);
    ret = this.sysFuncVideo(paramJson);
    return ret;
  };
  /**
   * @description 取消订阅实时图片信息
   * @example <caption>Example usage of <b>unSubscribeLivePic</b>.</caption>
   *  var nWndNo = 1;
   *      unSubscribeLivePic(nWndNo);
   * @param {Integer} nWndNo   图片显示窗口号
   * @param {String} nChlID	 通道ID
   * @param {String} nDevID	 设备ID
   * @returns {Object}
   ...
   *  data.action="UnSubCarInfo"
   */
  SOCXComm.prototype.unSubscribeLivePic = function (nWndNo) {
    var params = {}, paramJson, ret;
    if (!this.ocxObj) {
      throw new Error("OCX对象尚未初始化");
    }
    params.nWndNo = parseInt(nWndNo);
    paramJson = this.generateOcxParamJson("UnSubCarInfo", params);
    ret = this.sysFuncVideo(paramJson);
    return ret;
  };
  /**
  ...
  PlayerUtils.createPlayerDom = function (id, classId) {
    ...
    try {
      ...
      $playerDom.appendChild(this.createObjParam("quality", "high"));
      $playerDom.appendChild(this.createObjParam("allowScriptAccess", "always"));
      $playerDom.appendChild(this.createObjParam("allowNetworking", "internal"));
    } catch (e) {
  ...
  */
   ...
   */
  OCXUtils.inheritance(SPlayer, SOCXComm);
  /**
   ...
   */
  OCXUtils.inheritance(PCSOCXComm, OCXComm);
  /**
   * @description 注册控件消息回调函数，用于控件消息回调给web
   * @override 复写父类的系统回调函数  @see {@link OCXComm#registerCallback}
   * @param{function} callbackFunc    回调函数
  ...
  function RobotPlayer(id, target, classId) {
    /** @member {String} */
    this.id = id;
    /** @member {String} */
    this.target = target;
   ...
   * @description SPlayer继承SOCXComm
   */
  OCXUtils.inheritance(RobotPlayer, SOCXComm);
  /**
   * @description 是否初始化控件/控件是否可用
   * @type {boolean}
   */
  RobotPlayer.prototype.hasOcx = false;
  /**
   * @description 结构化OCX类型
   * @constant
   * @type {number}
   */
  RobotPlayer.prototype.EOCXTYPE = 5;
  /**
   * @todo 搞清楚初始化流程
   * @description 启动，加载控件
   * @type {function}
   */
  RobotPlayer.prototype.launch = function () {
    if (!this.ocxDom) {
      throw new Error("Create DOM Error！ --->OCX Object 标签创建失败！");
    }
    try {
      $(this.target).html(this.ocxObj);
      this.hasOcx = true;
    } catch (e) {
      this.hasOcx = false;
    }
  };
  RobotPlayer.prototype.openVideo = function (videoUrl) {
    this.ocxObj.Open(videoUrl);
  }
  /**
   * @description 关闭
   * @type {function}
   */
  RobotPlayer.prototype.closeVideo = function () {
    if (this.ocxObj.Close) {
      this.ocxObj.Close();
    }
  }
